...
def plot_colormesh(name, func, vmax = n_iter_max):
  ...
  for row in result:
    for col in row:
      for sim_result in col:
        scores_history = sim_result[2]
        if len(scores_history) == 0:
          quant_primeira += 1
          continue
        if sim_result[1] < 0.99:
          quant_nunca += 1
        if len(scores_history) < n_iter_max:
          last_score = scores_history[len(scores_history) - 1]
          scores_history += [last_score] * (n_iter_max - len(scores_history))
        plt.plot(range(1,n_iter_max+1), scores_history, linewidth=0.1, color='black')
        quant_normal += 1
